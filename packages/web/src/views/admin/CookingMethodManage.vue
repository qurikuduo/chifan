<template>
  <AppLayout title="烹饪方式管理" :show-back="true" :show-nav="false">
    <div class="add-form">
      <input class="input" v-model="newName" placeholder="新烹饪方式" />
...
async function load() {
  items.value = await api.get<Array<{ id: string; name: string }>>('/cooking-methods');
}

async function handleAdd() {
  if (!newName.value) return;
  await api.post('/cooking-methods', { name: newName.value });
  newName.value = '';
  await load();
}
...
async function handleDelete(id: string) {
  ...
  await api.delete(`/cooking-methods/${id}`);
  await load();
}
